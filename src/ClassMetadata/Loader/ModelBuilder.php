...
use Kassko\ObjectHydrator\ClassMetadata\ReflectionClass;
...
class ModelBuilder
{
    ...
    private function getValidatedConfig() : array
    {
        ...
        $classMetadataDefinition = new Loader\ClassMetadataDefinition;
    ...
    private function buildBaseProperty(Model\Property $property, array $propertyData, array $classData, \ReflectionProperty $reflectionProperty)
    {
        ...
        if ($this->isEnabled('callbacks_using_metadata', $propertyData)) {
            $property->setCallbacksUsingMetadata($this->buildCallbacks($propertyData['callbacks_using_metadata']));
        }

        if ($this->isEnabled('callbacks_hydration', $propertyData)) {
            $property->setCallbacksHydration($this->buildCallbacks($propertyData['callbacks_hydration']));
        }
...
/*
        ...
        if (isset($dataSourceData['callbacks_using_metadata'])) {
            $dataSource->setCallbacksUsingMetadata($this->buildCallbacks($dataSourceData['callbacks_using_metadata']));
        }

        if (isset($dataSourceData['callbacks_hydration'])) {
            $dataSource->setCallbacksHydration($this->buildCallbacks($dataSourceData['callbacks_hydration']));
        }
    ...
    private function buildCallbacks(array $propertyData) : Model\Callbacks
    {
        ...
        if ($this->hasCount('before_collection', $propertyData)) {
            foreach ($propertyData['before_collection'] as $beforeData) {
                $callbacks->addBefore($this->buildMethod($beforeData));
            }
        } elseif ($this->isEnabled('before', $propertyData)) {
            $callbacks->addBefore($this->buildMethod($propertyData['before']));
        }

        if ($this->hasCount('after_collection', $propertyData)) {
            foreach ($propertyData['after_collection'] as $afterData) {
                $callbacks->addAfter($this->buildMethod($afterData));
            }
        } elseif ($this->isEnabled('after', $propertyData)) {
